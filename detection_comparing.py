...
while(cap.isOpened()):
    ...
    #cv2.imshow('Convex Hull--WENYU ZHANG',drawing)
    cv2.imshow('Complex Background--WENYU ZHANG',drawing)
    #all_img = np.hstack((drawing, drawing2))
    #cv2.imshow('Hand Object Detection in Different Lightening Condition--WENYU ZHANG', all_img)
    k = cv2.waitKey(10)
    if k == 27:
        break